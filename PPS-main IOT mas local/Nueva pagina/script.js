...
let currentAdf4351StepIndex = 2; // Empezar en 1 kHz
...
let currentAd9850StepIndex = 3; // Empezar en 1 kHz
...
function formatStep(stepHz) {
    if (!stepHz || isNaN(stepHz)) return "--";
    const numStep = Number(stepHz);
    if (numStep >= 1000000) return (numStep / 1000000).toFixed(0) + ' MHz';
    if (numStep >= 1000) return (numStep / 1000).toFixed(0) + ' kHz';
    return numStep + ' Hz';
}
...
function actualizarPantallaAD9850(datos) { 
    ...
    // Actualizar paso actual si viene en los datos
    if (datos.paso_hz) {
        ad9850StepDisplay.textContent = formatStep(datos.paso_hz);
    }
}
...
function actualizarPantallaADF4351(datos) { 
    ...
    // Actualizar paso actual si viene en los datos
    if (datos.paso_hz) {
        adf4351StepDisplay.textContent = formatStep(datos.paso_hz);
    }
...
// ==========================================================
function cambiarPasoADF4351() {
    // Cambiar al siguiente paso
    currentAdf4351StepIndex = (currentAdf4351StepIndex + 1) % ADF4351_STEPS.length;
    const nuevoPaso = ADF4351_STEPS[currentAdf4351StepIndex];
    
    // Actualizar display local
    adf4351StepDisplay.textContent = formatStep(nuevoPaso);
    ...
    appendLog(`🔧 Paso ADF4351 cambiado a: ${formatStep(nuevoPaso)}`);
}

function ajustarFrecuenciaADF4351(direccion) {
    // Ya no es necesario enviar el 'paso_hz'. El ESP32 usará el que tiene almacenado.
    enviarComando({ 
        accion: "adf4351_command", 
        sub_accion: "change_freq", 
        direccion: direccion
    });
    
    // Mantenemos el log local para que el usuario vea qué paso se está usando.
    const pasoActual = ADF4351_STEPS[currentAdf4351StepIndex];
    appendLog(`🔧 Ajustando ADF4351: ${direccion === 'up' ? '+' : '-'}${formatStep(pasoActual)}`);
}
...
function cambiarPasoAD9850() {
    // Cambiar al siguiente paso
    currentAd9850StepIndex = (currentAd9850StepIndex + 1) % AD9850_STEPS.length;
    const nuevoPaso = AD9850_STEPS[currentAd9850StepIndex];
    
    // Actualizar display local
    ad9850StepDisplay.textContent = formatStep(nuevoPaso);
    ...
    appendLog(`🔧 Paso AD9850 cambiado a: ${formatStep(nuevoPaso)}`);
}

function ajustarFrecuenciaAD9850(direccion) {
    // Enviar comando de ajuste de frecuencia
    enviarComando({ 
        accion: "ad9850_command", 
        sub_accion: "change_freq", 
        direccion: direccion
    ...
    // Mantenemos el log local para que el usuario vea qué paso se está usando.
    const pasoActual = AD9850_STEPS[currentAd9850StepIndex];
    appendLog(`🔧 Ajustando AD9850: ${direccion === 'up' ? '+' : '-'}${formatStep(pasoActual)}`);
}
...
// --- Listeners de Controles AD9850 MEJORADOS ---
...
// --- Controles AD9850 con manejo de pasos ---
...
// --- Listeners de Controles ADF4351 MEJORADOS ---
...
// --- Controles ADF4351 con manejo de pasos ---
adf4351FreqUpBtn.addEventListener("click", () => {
    ajustarFrecuenciaADF4351('up');
});

adf4351FreqDownBtn.addEventListener("click", () => {
    ajustarFrecuenciaADF4351('down');
});
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Inicializar display de los pasos
